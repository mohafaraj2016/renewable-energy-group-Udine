...
export default function App() {
  ...
  const t = {
    en: {
      ...
      welcome: 'Welcome to the Renewable Energy Group at the University of Udine.', uploadCSV: 'Upload CSV',
      downloadSample: 'Download Sample', address: '123 Renewable Way, Udine, Italy',
      usefulLinks: 'Useful Links', social: 'Follow us on social media'
    },
    it: {
      ...
      welcome: "Benvenuti nel Gruppo di Energia Rinnovabile dell'Università di Udine.", uploadCSV: 'Carica CSV',
      downloadSample: 'Scarica Esempio', address: "Via dell'Energia 123, Udine, Italia",
      usefulLinks: 'Link Utili', social: 'Seguici sui social media'
    }
  };

  const heroImages = [
    { src: '/Hero_1.png', captionEn: 'Solar Energy Research', captionIt: 'Ricerca sull’Energia Solare' },
    { src: '/Hero_1.png', captionEn: 'Wind Tunnel Simulation', captionIt: 'Simulazione della Galleria del Vento' },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto font-sans">
      ...
      {/* Hero Section with Buttons and Captions */}
      <section className="relative h-64 bg-cover bg-center text-white flex flex-col items-center justify-center transition-all duration-1000" style={{ backgroundImage: `url('${heroImages[currentImageIndex].src}')` }}>
      ...
      <nav className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 text-white font-semibold">
        <a href="#about" className="bg-blue-600 hover:bg-blue-700 py-3 px-4 text-center rounded">{t[lang].about}</a>
        <a href="#research" className="bg-green-600 hover:bg-green-700 py-3 px-4 text-center rounded">{t[lang].research}</a>
        <a href="#members" className="bg-purple-600 hover:bg-purple-700 py-3 px-4 text-center rounded">{t[lang].members}</a>
        <a href="#publications" className="bg-pink-600 hover:bg-pink-700 py-3 px-4 text-center rounded">{t[lang].publications}</a>
        <a href="#news" className="bg-yellow-500 hover:bg-yellow-600 py-3 px-4 text-center rounded text-black">{t[lang].news}</a>
        <a href="#collaborations" className="bg-red-500 hover:bg-red-600 py-3 px-4 text-center rounded">{t[lang].collaborations}</a>
        <a href="#contact" className="bg-gray-700 hover:bg-gray-800 py-3 px-4 text-center rounded">{t[lang].contact}</a>
        <a href="#data" className="bg-teal-600 hover:bg-teal-700 py-3 px-4 text-center rounded">{t[lang].data}</a>
      </nav>
      ...
      <section id="data" className="p-6 md:p-10">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="border rounded p-4 flex-1">
            ...
            <button className="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">{t[lang].uploadCSV}</button>
          </div>
          <div className="border rounded p-4 flex-1">
            <h3 className="font-semibold mb-2">{t[lang].downloadSample}</h3>
            <button className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600">{t[lang].downloadSample}</button>
          </div>
        </div>
      </section>
        ...
        <div className="p-6 grid md:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <ul className="space-y-1">
              <li><a href="https://www.uniud.it" className="text-blue-600 hover:underline" target="_blank" rel="noopener noreferrer">University of Udine</a></li>
